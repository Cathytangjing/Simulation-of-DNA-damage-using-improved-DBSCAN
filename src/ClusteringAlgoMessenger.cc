...
ClusteringAlgoMessenger::ClusteringAlgoMessenger(ClusteringAlgo *pClustAlgo)
  ...
  fpChemProbCmd = new G4UIcmdWithADouble("/clustering/algo/setChemSelectionProb", this);
  fpChemProbCmd->SetGuidance("Probability to select potential "
                             "chemical damage according to the geometry ");
  fpChemProbCmd->SetParameterName("Prob", false);
  fpChemProbCmd->SetRange("Prob>0");
  fpChemProbCmd->AvailableForStates(G4State_Idle);
...
void ClusteringAlgoMessenger::SetNewValue(G4UIcommand *pCommand,
  ...
  if (pCommand == fpChemProbCmd) {
    fpClusteringAlgo->SetChemicalPointsProb(fpChemProbCmd->GetNewDoubleValue(pNewValue));
  }
