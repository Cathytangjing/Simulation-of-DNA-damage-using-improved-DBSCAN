...
void SteppingAction::UserSteppingAction(const G4Step *pStep) {
  ...
  if (particle_name == G4String("proton")) {
    event_action_->AddStepLength(step_length);
  }

  if (edepStep > 0 && target_volume == the_volume) {
    event_action_->AddEdep(edepStep);
    event_action_->clustering_->RegisterPhysicalDamage(pStep->GetPreStepPoint()->GetPosition(), edepStep);
  }
}
