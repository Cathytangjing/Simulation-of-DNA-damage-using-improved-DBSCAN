...
SBPoint::SBPoint(unsigned int pId, G4ThreeVector pPos, G4double pEdep) :
  ...
  strand_id_ = (G4UniformRand() < 0.5) ? 0 : 1;
}
...
bool SBPoint::operator[](int i) const {
  return position_[i];
}

bool SBPoint::operator==(const SBPoint &p) const {
  return position_[0] == p[0] && position_[1] == p[1] && position_[2] == p[2];
}
