...
void ClusterSBPoints::Clear() {
  std::set<SBPoint *>::iterator itPt;
  for (itPt = registered_points_.begin();
       itPt != registered_points_.end();
       ++itPt) {
    (*itPt)->CleanCluster();
  }
...
void ClusterSBPoints::AddSBPoint(SBPoint *pSBPoint) {
  assert(pSBPoint);
  registered_points_.insert(pSBPoint);
...
G4ThreeVector ClusterSBPoints::GetBarycenter() const {
  ...
  for (itSDSPt = registered_points_.begin();
       itSDSPt != registered_points_.end();
       ++itSDSPt) {
  ...
  return G4ThreeVector(
      x / registered_points_.size(),
      y / registered_points_.size(),
      z / registered_points_.size());
}
...
void ClusterSBPoints::FindAllPointsPossible(std::vector<SBPoint *> *pPtsToCheck,
  ...
  while (itPt != pPtsToCheck->end()) {
    ...
    if ((!(*itPt)->HasCluster())
        && (registered_points_.find(*itPt) == registered_points_.end())
...
bool ClusterSBPoints::HasIn(const SBPoint *pPtToCheck, G4double pMinDist) {
  ...
  for (itClusPt = registered_points_.begin();
       itClusPt != registered_points_.end();
       ++itClusPt) {
